...
export interface RecommendationRecord {
  ...
  /** JSONB — minimal Step 3 placeholder; replaced by AI content in Step 4. */
  explanationJson: MinimalExplanationJson;
}

/**
 * Minimal explanation JSON stored at Step 3.
 * The full AI-generated shape belongs to Step 4.
 */
export interface MinimalExplanationJson {
  summary: string;
...
export interface ScoredOpportunity {
  ...
  /** Placeholder explanation for Step 3 (AI content added in Step 4). */
  explanation: MinimalExplanationJson;
}
...
// ---------------------------------------------------------------------------
// 3.4 Internal helper — build minimal explanation for Step 3
// ---------------------------------------------------------------------------

/**
 * Builds the minimal placeholder explanation persisted at Step 3.
 * Step 4 will replace this with AI-generated text.
 *
 * @param opportunity  - The scored opportunity.
 * @param skillStates  - Student skill states (for gap computation).
 * @param scores       - The compatibility breakdown.
 */
function buildMinimalExplanation(
  opportunity: ScoringOpportunity,
  skillStates: StudentSkillState[],
  scores: _CompatibilityScores
): MinimalExplanationJson {
  ...
  for (const tag of opportunity.skillTags) {
    const skillName = tag.skill.name;
  ...
  return {
    summary: `${scoreLabel} (${(scores.total * 100).toFixed(1)}% compatibility). Step 4 AI explanation pending.`,
    matchingSkills,
    gapSkills,
  };
}
...
// ---------------------------------------------------------------------------
// 3.5 generateRecommendations() — main orchestration function
// ---------------------------------------------------------------------------
...
export async function generateRecommendations(
  ...
  const scoredOpportunities: ScoredOpportunity[] = opportunities.map((opp) => {
    ...
    const explanation = buildMinimalExplanation(opp, skillStates, scores);
  ...
  // ── F. Segment + rank ──────────────────────────────────────────────────
  const segmented = segmentResults<ScoredOpportunity>(scoredOpportunities);

  // ── G. Persist (upsert — idempotent) ──────────────────────────────────
  const allIncluded = [
    ...segmented.readyNow,
    ...segmented.almostReady,
    ...segmented.aspirational,
  ];
  ...
  return {
    readyNow:    segmented.readyNow,
    almostReady: segmented.almostReady,
    aspirational: segmented.aspirational,
    meta: {
      studentId,
...
// ---------------------------------------------------------------------------
// 3.6 Build result from cached persisted records
// ---------------------------------------------------------------------------
...
function buildResultFromCached(
  ...
  const toScoredOpportunity = (rec: PersistedRecommendation): ScoredOpportunity => {
    ...
    const explanation = (rec.explanationJson as MinimalExplanationJson | null) ?? {
      summary: '',
      matchingSkills: [],
      gapSkills: [],
    };
