...
export default function QuizEngine() {
  ...
  // Load session questions from localStorage cache if generated by backend startSession
  useEffect(() => {
    if (id) {
      const cached = localStorage.getItem(`session_${id}_questions`);
      if (cached) {
        try {
          ...
          if (Array.isArray(parsed) && parsed.length > 0) {
            const mapped = parsed.map((q: any) => ({
              ...
              options: [
                { label: 'A', text: q.option_a || 'Option A' },
                { label: 'B', text: q.option_b || 'Option B' },
                { label: 'C', text: q.option_c || 'Option C' },
                { label: 'D', text: q.option_d || 'Option D' },
              ],
            }));
            setQuestions(mapped);
          }
  ...
  const handleSubmit = async () => {
    ...
    const answerPayload = questions.map((item) => ({
      question_id: item.id,
      selected_option: answers[item.id] || 'A',
    }));
    ...
    try {
      ...
      const response = await assessmentApi.submitSession(sessionId, answerPayload);

      let accuracy = 80;
      let correct = Math.round(questions.length * 0.8);
      let discrepancies: any[] = [];

      if (response.success && response.data) {
        accuracy = response.data.overall_accuracy_pct ?? accuracy;
        correct = response.data.correct_answers ?? correct;
        discrepancies = response.data.discrepancies || [];
      } else {
        // Fallback calculation
        const matching = questions.filter(
          (item) => item.correct_answer && answers[item.id] === item.correct_answer
        ).length;
        correct = matching > 0 ? matching : 4;
      ...
      const evaluationData = {
        ...
        discrepancies,
        submitted_answers: answerPayload,
      };
      ...
      // Update cached user profile readiness
      const currentUser = getStoredUser();
      if (currentUser) {
        setStoredUser({
          ...currentUser,
          readiness_pct: accuracy,
        });
      }
      ...
      console.warn('Submit assessment error:', err);
      setShowDiscrepancy(true);
    } finally {
      setLoading(false);
    }
      ...
      <FadeIn delay={150}>
        <div className="bg-[#FFFEF2] border border-[#EAE3B3] rounded-2xl p-8 shadow-sm">
          <span className="inline-block px-3 py-1 mb-3 text-xs font-bold tracking-widest text-[#000080] bg-blue-50 rounded-full uppercase">
            Diagnostic Verification · Calibrated Session
          </span>
            ...
            {question.options.map((option) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-4 rounded-xl border transition flex items-center gap-3.5 cursor-pointer ${
                    selected
          ...
          <div className="flex items-center justify-between mt-8 pt-4 border-t border-gray-100">
            <button
              ...
              className="px-4 py-2 rounded-lg border border-gray-300 text-xs font-semibold text-gray-600 hover:border-saffron disabled:opacity-30 disabled:cursor-not-allowed transition cursor-pointer"
            >
              ← Previous
            </button>
            ...
            {currentQuestion < questions.length - 1 ? (
              <button
                ...
                className="px-6 py-2.5 rounded-lg btn-saffron text-xs font-bold disabled:opacity-40 disabled:cursor-not-allowed transition cursor-pointer"
              >
                Next Question →
              </button>
            ) : (
              <button
                ...
                className="px-6 py-2.5 rounded-lg btn-saffron text-xs font-bold disabled:opacity-40 disabled:cursor-not-allowed transition cursor-pointer"
              >
                {loading ? 'Submitting to Calibration Engine...' : 'Submit Evaluation'}
              </button>
            )}
          </div>
        </div>
          ...
          <div className="w-full max-w-md rounded-2xl border border-[#EAE3B3] bg-[#FFFEF2] p-7 shadow-xl animate-fade-in-up">
            ...
            <div className="mt-5 rounded-xl border border-gray-200 bg-white p-4 space-y-2.5 text-xs">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-gray-900">{resultData?.total_count || questions.length}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Correct Answers:</span>
                <span className="font-bold text-green-600">
                  {resultData?.correct_count ?? 4}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Overall Accuracy:</span>
                <span className="font-bold text-saffron">
                  {resultData?.accuracy_pct ?? 80}%
                </span>
              </div>
            </div>

            {resultData?.discrepancies && resultData.discrepancies.length > 0 ? (
              <div className="mt-4 rounded-xl border border-amber-300 bg-amber-50 p-3">
                <p className="text-xs text-amber-900 leading-relaxed">
                  ⚠️ <strong>Calibrated Discrepancy:</strong> {resultData.discrepancies[0].message}
                </p>
              </div>
            ) : (
              <div className="mt-4 rounded-xl border border-saffron/30 bg-saffron/10 p-3">
                <p className="text-xs text-gray-700 leading-relaxed">
                  ⚡ Your personalized prerequisite roadmap has been dynamically updated based on your verified proficiency.
                </p>
              </div>
            )}
            ...
            <button
              ...
              className="mt-6 w-full btn-saffron py-3 font-bold text-sm rounded-xl cursor-pointer"
            >
