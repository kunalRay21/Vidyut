...
/**
 * Intelligent matching function: Matches an input query against the pre-defined
 * questions library using keyword tokenization, substring matching, and tag overlap.
 */
export function matchQuestion(queryStr: string): {
  matchedQuestion: HelpQuestion | null;
  candidates: HelpQuestion[];
} {
  const cleanQuery = queryStr.trim().toLowerCase();
  if (!cleanQuery) {
    return { matchedQuestion: null, candidates: HELP_QUESTIONS.slice(0, 5) };
  }
  ...
  // Filter query tokens excluding stop words
  ...
  // Require a score threshold that indicates real domain relevance (at least 8)
  ...
  // No strong match: return top 4 general suggestion questions
  return {
    matchedQuestion: null,
    candidates: HELP_QUESTIONS.slice(0, 4),
  };
